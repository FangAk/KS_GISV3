...
export { uploadImgFunction } from './uploadFile'
/**
 * render 图标
 * */
...
export function require(url: string) {
  try {
    const handlePath = url.replace('@', '')
    return new URL(handlePath, import.meta.url).href
  } catch (err) { }
}
/**
 * 数组坐标转字符串坐标
 * @param pointArr 需要转换的数组坐标
 **/
export const arrToStr = (pointArr) => {
  let strPoint = "";
  pointArr
    ?.map((item) => `${item[0]?.toFixed(2)},${item[1]?.toFixed(2)}`)
    .forEach((item) => (strPoint += `${item};`));
  return strPoint;
};